...
#import all data files and combine into one data frame 
ridership_files = list.files(pattern="*.tsv")
allData1 <- lapply(ridership_files, read.delim)
cta_data <- do.call(rbind, allData1)

#extra year and day of the week as new columns for each entry using lubridate functions
cta_data$datestamp <- as.Date(cta_data$date, "%m/%d/%Y") 
cta_data$year <- year(cta_data$datestamp)
cta_data$month <- month(cta_data$datestamp, label=TRUE)
cta_data$dayOfWeek <-wday(cta_data$datestamp, label=TRUE)
...
ui <- fluidPage(
  ...
  titlePanel("CS 424 Spring 2022 Project 1 - Subway"),
  fluidRow(
...
server <- function(input, output) {
  ...
  # a bar chart showing total entries at station for each year (2001, 2002, ... 2021)
